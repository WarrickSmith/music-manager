    ...
    <div>
      ...
      <div className="space-y-4 mb-6">
        ...
        <div className="w-full bg-sky-50 border border-sky-100 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sky-600 font-semibold">Filter Music Files</span>
          </div>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
            <div className="col-span-1 sm:col-span-1">
              <label className="text-sm font-medium text-sky-500 mb-1 block">Year</label>
              <Select value={selectedYear} onValueChange={handleYearChange}>
                ...
                <SelectContent className="bg-white border-sky-200">
                  <SelectItem value="all" className="text-sky-600 font-medium">All Years</SelectItem>
                  {uniqueYears.map((year) => (
                    <SelectItem key={year} value={year.toString()} className="text-sky-600">
                      {year}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="col-span-1 sm:col-span-1 md:col-span-2">
              <label className="text-sm font-medium text-indigo-500 mb-1 block">Competition</label>
              <Select value={selectedCompetition} onValueChange={handleCompetitionChange}>
                ...
                <SelectContent className="bg-white border-indigo-200 max-h-60">
                  <SelectItem value="all" className="text-indigo-600 font-medium">All Competitions</SelectItem>
                  {uniqueCompetitions.map((comp) => (
                    <SelectItem key={comp} value={comp} className="text-indigo-600">
                      {comp}
                    </SelectItem>
                  ))}
                </SelectContent>
      ...
      {filteredFiles.length === 0 ? (
        <div className="text-center py-6 bg-sky-50 rounded-lg border border-sky-200">
          <p className="text-sky-500 mb-2">No files match the selected filters</p>
          <p className="text-sky-400 text-sm">Try selecting different filter options</p>
        </div>
      ) : (
